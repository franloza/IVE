...
The first stage of our experiment, the one carried out in 2D, will consist in sitting the subject in front of one laptop with the experiment running full screen. The subject will be allow to change the view of the graph played in the screen using the mouse (the same efect that accomplished using the headtracking in the next two stages). This also will be helpful in order to place the subject in the position wanted (sitting and no farther than one meter from the laptop).

This stage will teach how answer in each graph, by using the left or the right click in the mouse and that once an answer is given the next graph will be played. These will remain for the whole experiment, so the subject could get used to it since the beginning, which will result useful because with the oculus on the subject will have enough new stimulus. 
